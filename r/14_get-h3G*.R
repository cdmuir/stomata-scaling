# Plots of affect on (co)variance of G* in H3
source("r/header.R")
...
df1 = crossing(
  ...
  M_aS  = G_h3_pars$M_aS,
  r_M   = G_h3_pars$r_M,
  omega = G_h3_pars$omega,
  phi_a = G_h3_pars$phi_a,
  beta  = G_h3_pars$beta
) |>
  mutate(M_dSaS = r_M * sqrt(M_dS * M_aS)) |>
  rowwise() |>
  mutate(M = list(matrix(c(
    M_dS, rep(M_dSaS, 2), M_aS
  ), 2, 2, byrow = TRUE)), G_star = list(get_Gstar(phi_a, omega, M, beta, model = "h3"))) |>
  filter(!is.na(G_star[1])) |>
  mutate(G_dS = G_star[1, 1],
         G_aS = G_star[2, 2],
         G_dSaS = G_star[1, 2])
...
df2 = bind_rows(
  df1 |>
    filter(round(r_M, 1) == 0, phi_a == 2, M_aS == micro_pars$M[2, 2]) |>
    select(M = M_dS, G = G_dS, omega) |>
    mutate(trait = "density"),
  df1 |>
    filter(round(r_M, 1) == 0, phi_a == 2, M_dS == micro_pars$M[1, 1]) |>
    select(M = M_aS, G = G_aS, omega) |>
    mutate(trait = "size")
) |>
  mutate(`Strength of\nselection on $g_\\mathrm{s,max}$` = factor(
    case_when(
...
figH3G1 = ggplot(df2,
  ...
  scale_y_log10(
    limits = c(2e-4, 2e-3),
    breaks = c(2.5e-4, 0.5e-3, 1e-3, 2e-3),
    labels = c(
      "$0.25 \\times 10^{-3}$",
      "$0.50 \\times 10^{-3}$",
      "$1.00 \\times 10^{-3}$",
...
# Effect of mutational correlation and phi_a on trait correlation
figH3G2 = df1 |>
  filter(M_dS == micro_pars$M[1, 1],
         M_aS == micro_pars$M[2, 2],
         phi_a %in% c(0.5, 1, 2))  |>
  mutate(
    r_G = G_dSaS / sqrt(G_dS * G_aS),
    `Strength of\nselection on\nstomatal size` = factor(
      case_when(phi_a == 0.5 ~ "strong", phi_a == 1 ~ "medium", phi_a == 2 ~ "weak"),
...
figH3G3 = df1 |>
  ...
  ggplot(aes(phi_a, rel_var, linetype = `Mutational variance\nof stomatal size`)) +
  geom_line(linewidth = 1.1) +
  xlab("Strength of selection on stomatal size") +
  ylab("Relative genetic variance, $G^*_{d_\\mathrm{S}} / G^*_{a_\\mathrm{S}}$") +
  scale_x_log10(breaks = c(0.5, 1, 2),
                labels = paste0("$", c(0.5, 1, 2), "\\omega$")) +
  scale_y_log10(limits = c(1 / 5, 5), breaks = c(1 / 4, 1, 4)) +
  coord_equal()

# Write parameters and figures
tikz(file = "ms/figures/figH3G1.tex",
     width = 6,
     height = 4)
print(figH3G1)
dev.off()

tikz(file = "ms/figures/figH3G2.tex",
     width = 6,
     height = 4)
print(figH3G2)
dev.off()

tikz(file = "ms/figures/figH3G3.tex",
     width = 6,
     height = 4)
print(figH3G3)
dev.off()

write_rds(G_h3_pars, "objects/G_h3_pars.rds")
